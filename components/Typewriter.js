...
const Typewriter = ({ actions }) => {
    ...
    const handleTyping = async (textToType) => {
        for (let i = 0; i < textToType.length; i++) {
            await new Promise((resolve) => {
                setTimeout(() => {
                    setText((prevText) => {
                        // Append the new character to the end of the existing text
                        const newText = prevText + textToType.charAt(i);
                        return newText;
                    });

                    // Update the cursor position to the end of the new text
                    setCursorPos((prevPos) => prevPos + 1);
    ...
    const renderTextAndCursor = (characters, cursorPosition) => {
        const renderedContent = characters.map((char, index) => (
            <span key={index}>
                {char}
            </span>
        ));

        // Append the cursor at the end of the rendered content
        renderedContent.push(<span key={characters.length} className={styles.cursor}></span>);
    ...
    return (
        <div>
            <h2>{renderTextAndCursor(splitTextIntoCharacters(text), cursorPos)}</h2>
        </div>
    );
};

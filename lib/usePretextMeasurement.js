...
export function usePretextMeasurement({ text, variant, whiteSpace }) {
  ...
  const { alignmentSample, lineHeight, measureFont } = config;
    ...
    async function measureText() {
      if (!text || width <= 0) {
        setMeasurement({
          height: 0,
          lineCount: text ? 1 : 0,
          offsetY: getGridTextOffset({ alignmentSample, lineHeight, measureFont }),
          ready: true,
        });
        return;
      }
      ...
      const offsetY = getGridTextOffset({ alignmentSample, lineHeight, measureFont });
  ...
  }, [alignmentSample, effectiveWhiteSpace, lineHeight, measureFont, text, variant, width]);
...
function getGridTextOffset(config) {
  const cacheKey = `${config.measureFont}__${config.lineHeight}__${config.alignmentSample}`;
  ...
  const offsetY = Math.round(desiredTopInset - currentTopInset);

...
export function usePretextMeasurement({ text, variant, whiteSpace, singleLineFit }) {
  ...
  useEffect(() => {
    ...
    async function measureText() {
      ...
      setMeasurement({
        ...
        inkOffsetX: resolvedConfig.inkOffsetX,
      });
  ...
  return {
    config: measurement.ready
      ? {
          ...config,
          characterTrackingX: measurement.characterTrackingX ?? config.characterTrackingX,
          inkOffsetX: measurement.inkOffsetX ?? config.inkOffsetX,
        }
      : config,
...
function createDotmatrixConfig(config, gridSize, rows, measureFontFamily) {
  const rowScale = rows / DOTMATRIX_MAX_ROWS;
  const lineHeight = gridSize * rows;
  const fontSize = (gridSize * rows) / DOTMATRIX_GLYPH_RATIO;
  const characterTrackingX = gridSize * 0.375 * rowScale;
  const inkOffsetX = (-gridSize / 3) * rowScale;
  const inkOffsetY = Math.round(gridSize * 0.15 * rowScale);
